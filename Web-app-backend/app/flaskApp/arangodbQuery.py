...
def getUsageOverTime(db, document, date='2015-01-01', graph='github'):
    aql_usage = "LET startDate = @date " \
                ...
                "FOR date IN dates LET count = LENGTH(( FOR v, e, p IN 2 INBOUND @document GRAPH @graph FILTER DATE_DIFF(p.vertices[1].date, date, 'd', true) > 0 RETURN DISTINCT v ))  " \
                "SORT date " \
...
def getRelevantLibrary(db, document, graph='github'):
    aql_relevant = "LET users =  (FOR v, e, p IN 2 INBOUND @document GRAPH @graph RETURN DISTINCT v) " \
                   "FOR user IN users " \
                   "    LET revision = (FOR v IN 1 OUTBOUND user GRAPH @graph SORT v.date DESC RETURN v)[0] " \
                   "    LET docs = (FOR library IN 1 OUTBOUND revision GRAPH @graph FILTER library != DOCUMENT(@document)  RETURN library) " \
                   "FOR doc IN docs " \
...
def getLibrary(db, libraryID):
    return db.AQLQuery("RETURN DOCUMENT(@libraryID)", bindVars={'libraryID':libraryID}, batchSize=1, rawResults=True)[0]
